...
describe 'Payments::StripeController', type: :request do
  describe 'POST /payments/stripe/update_status' do
    ...
    let(:session) { double('Stripe::Checkout::Session', id: 'cs_test_123', payment_status: 'paid') }
    let(:event_data) { double('Stripe::StripeObject', object: session) }
    let(:event) { double('Stripe::Event', type: 'checkout.session.completed', data: event_data) }
    ...
    context 'failure path' do
      context 'when HandleWebhookService fails' do
        ...
        it 'returns unprocessable entity and reports service errors to Rollbar' do
          subject
      ...
      context 'when event verification fails' do
        ...
        it 'returns unprocessable entity and reports error to Rollbar' do
          subject
